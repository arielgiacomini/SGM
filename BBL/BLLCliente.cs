...
namespace BBL
{
    public class BLLCliente
    {
        ...
        {
            if (modelo.CCliente.Trim().Length == 0 | modelo.CApelido.Trim().Length == 1 && modelo.CApelido.Trim().Length == 0 | modelo.CCliente.Trim().Length == 1)
            {
                throw new Exception("O nome do cliente ou Apelido é obrigatório.");
            }

            if (modelo.CEmail.Trim().Length == 0)
            {
